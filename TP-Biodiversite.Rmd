...
```{r ComptageVisible, ref.label="Comptage"}
```

Produire un diagramme de Whittaker avec ggplot, en ajustant une distribution lognormale.

```{r Whittaker, exercise.setup="Comptage", exercise=TRUE, hint=TRUE}
library("entropart")
...
```{r Richesse1, exercise.setup="Comptage", exercise=TRUE, hint=TRUE}
...
```{r Singletons, exercise.setup="Comptage", exercise=TRUE, hint=TRUE}
...
```{r Richesse2, exercise.setup="Comptage", exercise=TRUE}
AbdP6 %>%  Richness(Correction = "None")
```

Estimer la richesse avec les estimateurs de Chao et Jackknife

```{r Richesse3, exercise.setup="Comptage", exercise=TRUE, hint=TRUE}
...
```{r SPECIES, exercise.setup="Comptage", exercise=TRUE}
library("SPECIES")
...
```{r Shannon1, exercise.setup="Comptage", exercise=TRUE}
AbdP6 %>% Shannon()
AbdP6 %>% Diversity(q=1)
```
...
```{r Shannon2, exercise.setup="Comptage", exercise=TRUE, hint=TRUE}
...
```{r Diversityq2, exercise.setup="Comptage", exercise=TRUE}
AbdP6 %>% Diversity(q=2)
# Compléter le code
```
...
```{r Profil1, exercise.setup="Comptage", exercise=TRUE}
CommunityProfile(Diversity, AbdP6) %>% autoplot
```

Comparer avec le profil estimé naïvement.

```{r Profil2, exercise.setup="Comptage", exercise=TRUE, message=FALSE}
...
```{r Leveln, exercise.setup="Comptage", exercise=TRUE}
...
```{r Coverage, exercise.setup="Comptage", exercise=TRUE}
Coverage(AbdP6)
```
...
```{r LevelC, exercise.setup="Comptage", exercise=TRUE}
...
```{r DivAC, exercise.setup="Comptage", exercise=TRUE, exercise.timelimit=120}
...
```{r p6PhyloVisible, ref.label="Comptage"}
```

Afficher la phylogénie.

```{r phylogenie, exercise=TRUE, hint=TRUE, exercise.timelimit=120, exercise.setup="p6Phylo"}
...
```{r phyShannon, exercise.setup="p6Phylo", exercise=TRUE, exercise.timelimit=120}
...
```{r phyProfil, exercise.setup="p6Phylo", exercise=TRUE, exercise.timelimit=120}
